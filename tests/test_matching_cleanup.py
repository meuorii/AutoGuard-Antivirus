...
def confirmed_incident(tmp_path, incidents, quarantine, detector):
    seed_dir = tmp_path / "seed"; seed_dir.mkdir(exist_ok=True)
    seed = seed_dir / "confirmed-threat.bin"; seed.write_bytes(AUTOGUARD_TEST_CONTENT)
...
def test_identical_content_with_different_filenames_is_quarantined(setup, tmp_path):
    ...
    assert result.sha256 == AUTOGUARD_TEST_SHA256
    assert len(result.locations_searched) == 4
    assert result.candidates_examined == 3 and result.exact_matches_found == 2 and result.matches_quarantined == 2 and result.already_contained_matches == 0
    assert result.failures == () and result.inaccessible_files == ()
...
def test_threat_trail_candidate_is_revalidated_and_not_processed_twice(setup, tmp_path):
    ...
    observation = trail.record_file(copy, ScanSource.USB)
    assert observation.sha256 == AUTOGUARD_TEST_SHA256
...
def test_stale_threat_trail_observation_is_not_trusted_without_current_hash(setup, tmp_path):
    ...
    trail.record_file(path, ScanSource.USB)
    path.write_bytes(b"content changed after historical observation")
...
def test_inaccessible_candidate_is_reported_without_cleanup(setup, tmp_path, monkeypatch):
    ...
    assert result.inaccessible_files[0].path == str(normalize_path(locked))
    assert "Permission denied" in result.inaccessible_files[0].reason and locked.exists()
...
def test_quarantine_failure_is_returned_in_structured_failures(setup, tmp_path, monkeypatch):
    ...
    def fail_quarantine(*args, **kwargs): raise OSError("simulated quarantine backend failure")

    monkeypatch.setattr(quarantine, "quarantine_file", fail_quarantine)
    result = cleanup.cleanup_incident(incident.id)
...
def test_same_path_can_be_quarantined_again_only_when_content_is_currently_present(setup, tmp_path):
    ...
    assert first.matches_quarantined == 1 and not path.exists()
    item_count = len(quarantine.list_quarantined_items())
    ...
    third = cleanup.cleanup_incident(incident.id)
    assert third.exact_matches_found == 1 and third.matches_quarantined == 1 and not path.exists()
    assert len(quarantine.list_quarantined_items()) == item_count + 1