...
def setup(tmp_path):
    ...
    detection = signature_detector.detect_file(source); assert detection.status is DetectionStatus.HIGH_CONFIDENCE
...
def test_successful_recovery_verifies_and_restores_without_removing_quarantine_evidence(setup):
    ...
    assert result.status is RecoveryStatus.RESTORED; assert result.restored
    assert source.read_bytes() == AUTOGUARD_TEST_CONTENT; assert Path(item.stored_path).read_bytes() == AUTOGUARD_TEST_CONTENT
...
def test_integrity_failure_blocks_restore_and_marks_incident_investigating(setup):
    ...
    service = safe_recovery(database, incidents, quarantine)
    result = service.restore(item.quarantine_id)
    assert result.status is RecoveryStatus.INTEGRITY_FAILED; assert not source.exists()
...
def test_destination_collision_never_overwrites_existing_content(setup):
    ...
    service = safe_recovery(database, incidents, quarantine)
    result = service.restore(item.quarantine_id)
    assert result.status is RecoveryStatus.DESTINATION_CONFLICT; assert source.read_bytes() == existing
...
def test_missing_quarantine_object_is_safe_failure(setup):
    ...
    service = safe_recovery(database, incidents, quarantine)
    result = service.restore(item.quarantine_id)
    assert result.status is RecoveryStatus.MISSING_QUARANTINE_OBJECT; assert not source.exists()
    assert incidents.get_incident(item.incident_id).incident.status is IncidentStatus.INVESTIGATING
...
def test_restore_to_user_selected_alternative_location(setup, tmp_path):
    ...
    assert result.status is RecoveryStatus.RESTORED; assert destination.read_bytes() == AUTOGUARD_TEST_CONTENT
    assert not source.exists(); assert result.restored_path == str(normalize_path(destination))
    assert incidents.get_incident(item.incident_id).incident.status is IncidentStatus.RESTORED
...
def test_current_dangerous_detection_returns_strong_block_and_writes_nothing(setup):
    ...
    result = service.restore(item.quarantine_id)
    assert result.status is RecoveryStatus.DANGEROUS_BLOCKED; assert result.detection is not None
    ...
    assert not source.exists(); assert Path(item.stored_path).exists()
    assert incidents.get_incident(item.incident_id).incident.status is IncidentStatus.CONTAINED
...
def test_recovery_attempt_metadata_persists_after_reopening_database(setup, tmp_path):
    ...
    assert len(attempts) == 1; assert attempts[0].id == result.attempt.id
    assert attempts[0].status is RecoveryStatus.RESTORED; assert attempts[0].restored_path == str(normalize_path(destination))
