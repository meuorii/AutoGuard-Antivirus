...
def setup(tmp_path):
    ...
    signature_detector = Detector(load_signatures())

    source = tmp_path / "downloads" / "sample.txt"
    source.parent.mkdir()
    source.write_bytes(AUTOGUARD_TEST_CONTENT)
    ...
    item = quarantine.quarantine_detection(
        detection, incident.id, ScanSource.MANUAL, original_size=len(AUTOGUARD_TEST_CONTENT)
    )
...
def safe_recovery(database, incidents, quarantine):
    # Current definitions no longer contain the harmless test hash.
...
def test_successful_recovery_verifies_and_restores_without_removing_quarantine_evidence(setup):
    ...
    assert result.status is RecoveryStatus.RESTORED
    assert result.restored
    assert source.read_bytes() == AUTOGUARD_TEST_CONTENT
    ...
    assert len(attempts) == 1 and attempts[0].restored_path == str(normalize_path(source))
    events = incidents.get_incident(item.incident_id).events
    assert any(e.event_type is IncidentEventType.RECOVERY_ATTEMPTED for e in events)
    assert any(e.event_type is IncidentEventType.RECOVERY_SUCCEEDED for e in events)
...
def test_integrity_failure_blocks_restore_and_marks_incident_investigating(setup):
    ...
    assert result.status is RecoveryStatus.INTEGRITY_FAILED
...
def test_destination_collision_never_overwrites_existing_content(setup):
    database, incidents, quarantine, item, source, _ = setup
    existing = b"important existing user data"
    source.write_bytes(existing)
    service = safe_recovery(database, incidents, quarantine)

    result = service.restore(item.quarantine_id)

    assert result.status is RecoveryStatus.DESTINATION_CONFLICT
    assert source.read_bytes() == existing
    assert result.conflict_options == (
        RecoveryConflictOption.RESTORE_TO_ANOTHER_FILENAME,
        RecoveryConflictOption.CHOOSE_ANOTHER_DESTINATION,
        RecoveryConflictOption.CANCEL,
    )
...
def test_missing_quarantine_object_is_safe_failure(setup):
    ...
    assert result.status is RecoveryStatus.MISSING_QUARANTINE_OBJECT
    assert not source.exists()
    assert incidents.get_incident(item.incident_id).incident.status is IncidentStatus.INVESTIGATING
...
def test_restore_to_user_selected_alternative_location(setup, tmp_path):
    ...
    assert result.status is RecoveryStatus.RESTORED
    assert destination.read_bytes() == AUTOGUARD_TEST_CONTENT
    assert not source.exists()
    assert result.restored_path == str(normalize_path(destination))
    assert incidents.get_incident(item.incident_id).incident.status is IncidentStatus.RESTORED
...
def test_current_dangerous_detection_returns_strong_block_and_writes_nothing(setup):
    ...
    assert result.status is RecoveryStatus.DANGEROUS_BLOCKED
...
def test_recovery_attempt_metadata_persists_after_reopening_database(setup, tmp_path):
    ...
    reopened = RecoveryService(
        reopened_db,
        QuarantineService(reopened_db, quarantine.quarantine_dir, IncidentService(reopened_db)),
        Detector(SignatureStore()),
        IncidentService(reopened_db),
    )
    attempts = reopened.list_attempts(item.quarantine_id)

    assert len(attempts) == 1
    assert attempts[0].id == result.attempt.id
    assert attempts[0].status is RecoveryStatus.RESTORED
    assert attempts[0].restored_path == str(normalize_path(destination))


def test_unknown_quarantine_id_is_explicit_error(setup):
    database, incidents, quarantine, _, _, _ = setup
    service = safe_recovery(database, incidents, quarantine)
    with pytest.raises(KeyError, match="Unknown quarantine item"):
