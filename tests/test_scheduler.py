...
from app.scheduler import DispatchStatus, ScanKind, ScanScheduler, ScheduleSettings


class FakeBackend:
    def __init__(self): self.jobs, self.started, self.shutdown_calls = [], False, []
    def add_job(self, func, trigger, **kwargs): self.jobs.append((func, trigger, kwargs))
    def start(self): self.started = True
    def shutdown(self, wait=True): self.shutdown_calls.append(wait); self.started = False
...
class BlockingScanner(RecordingScanner):
    ...
    def scan(self, path, source, *, scan_type, interrupt_check=None):
        ...
        while not self.release.wait(0.01):
            if interrupt_check is not None and interrupt_check(): break
...
def test_schedule_defaults_are_24_hours_and_7_days(tmp_path):
    backend, scanner = FakeBackend(), RecordingScanner()
    scheduler = ScanScheduler(scanner, quick_paths=(tmp_path,), full_paths=(tmp_path,), scheduler_factory=lambda: backend)
    ...
    assert quick["hours"] == 24.0 and full["days"] == 7.0
...
def test_custom_scheduling_configuration_is_forwarded_to_apscheduler(tmp_path):
    ...
    scheduler = ScanScheduler(RecordingScanner(), quick_paths=(tmp_path,), full_paths=(tmp_path,), settings=settings, scheduler_factory=lambda: backend)
    scheduler.start()
    jobs = {kwargs["id"]: kwargs for _, _, kwargs in backend.jobs}
    assert jobs[ScanScheduler.QUICK_JOB_ID]["hours"] == 6 and jobs[ScanScheduler.FULL_JOB_ID]["days"] == 3
    scheduler.stop()
...
def test_startup_scan_is_dispatched_in_background_with_startup_history_metadata(tmp_path):
    ...
    assert dispatch.status is DispatchStatus.DISPATCHED and wait_until(lambda: len(scanner.calls) == 1)
    path, source, scan_type, _ = scanner.calls[0]
    assert path == tmp_path and source is ScanSource.STARTUP and scan_type is ScanType.STARTUP
...
def test_duplicate_quick_scan_is_prevented_while_startup_quick_scan_runs(tmp_path):
    ...
    assert duplicate.status is DispatchStatus.SKIPPED_ALREADY_RUNNING and len(scanner.calls) == 1
    scanner.release.set()
    assert wait_until(lambda: not scheduler.health().quick_scan_running)
...
def test_quick_and_full_scans_may_run_independently(tmp_path):
    ...
    assert full.accepted and wait_until(lambda: len(scanner.calls) == 2)
    scanner.release.set()
    assert wait_until(lambda: not scheduler.health().quick_scan_running and not scheduler.health().full_scan_running)


def test_scheduled_quick_scan_uses_scheduled_source_and_quick_scan_type(tmp_path):
    scanner = RecordingScanner()
    scheduler = ScanScheduler(scanner, quick_paths=(tmp_path,), full_paths=(tmp_path,))
    assert scheduler.dispatch_quick_scan().accepted and wait_until(lambda: len(scanner.calls) == 1)
    _, source, scan_type, _ = scanner.calls[0]
    assert source is ScanSource.SCHEDULED and scan_type is ScanType.QUICK


def test_scheduler_stop_shuts_down_backend_and_requests_worker_interrupt(tmp_path):
    backend, scanner = FakeBackend(), BlockingScanner()
    scheduler = ScanScheduler(scanner, quick_paths=(tmp_path,), full_paths=(tmp_path,), scheduler_factory=lambda: backend)
    scheduler.start()
    scheduler.dispatch_startup_scan()
    assert scanner.entered.wait(1.0)
    health = scheduler.stop(wait=True, timeout=1.0)
    assert backend.shutdown_calls == [True] and not health.running
    assert wait_until(lambda: not scheduler.health().quick_scan_running)