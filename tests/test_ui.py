...
def test_controller_dispatches_manual_scan_off_calling_thread():
    ...
    try:
        ...
        assert progress.payload["processed"] == 1 and progress.payload["discovered"] == 1 and progress.payload["progress"] == 1.0 and message.payload["result"].session_id == "session-1"
...
def test_ux_refresh_phase2_home_uses_only_four_user_facing_protection_states():
    ...
    try:
        ...
        att = controller._dashboard_snapshot()["protection_state"]
        assert att["title"] == "Attention needed" and att["message"] == "AutoGuard found something that needs review."
        controller.services = _home_services(file_running=False)
        iss = controller._dashboard_snapshot()["protection_state"]
        assert iss["title"] == "Protection issue" and iss["message"] == "One or more protection services are not running."
    finally: controller.shutdown()
...
def test_ux_refresh_phase3_idle_scan_page_uses_simple_user_facing_choices():
    ...
    assert all(req in source for req in ('text="Scan your PC"', 'title="Quick Scan"', 'title="Full Scan"', 'title="Custom Scan"', 'recommended=True', "Downloads  •  Desktop  •  Documents", '"Start Quick Scan", self.controller.start_quick_scan', '"Start Full Scan", self.controller.start_full_scan', '"Choose File", self._choose_file', '"Choose Folder", self._choose_folder'))
...
def test_ux_refresh_phase4_active_scan_is_focused_and_has_no_fake_pause():
    ...
    assert all(req in source for req in ("Files checked", "Threats found", "Currently checking", "More details", "Discovered", "Processed", "Skipped", "Errors", "Suspicious", "Dangerous", "Stop Scan"))
...
def test_ux_refresh_phase4_scan_page_uses_active_component_and_stop_controller():
    source = (Path(__file__).resolve().parents[1] / "app/ui/scan_page.py").read_text(encoding="utf-8")
    assert all(req in source for req in ("ActiveScanPanel", "on_stop=self.controller.stop_scan", 'message.kind == "scan_stop_requested"', 'message.kind == "scan_stopped"'))
...
def test_ux_refresh_phase4_stop_scan_uses_cooperative_interrupt_without_blocking_ui():
    ...
    try:
        controller.start_scan("large-folder"); assert scanner.worker_started.wait(1.0)
        started = time.monotonic(); assert controller.stop_scan() is True and time.monotonic() - started < 0.05
...
def test_ux_refresh_phase5_confirmed_result_claims_handled_only_with_verified_containment():
    ...
    services, _ = _phase5_result_services(session=session, results=(SimpleNamespace(detection_status=DetectionStatus.HIGH_CONFIDENCE, sha256=sha256),), incident=SimpleNamespace(id="internal-incident", sha256=sha256, status=IncidentStatus.CONTAINED), incident_events=(SimpleNamespace(event_type=IncidentEventType.MATCHING_COPY_FOUND, occurred_at=session.started_at + timedelta(seconds=4)),) * 2, verification=SimpleNamespace(status=VerificationStatus.VERIFIED))
...
def test_ux_refresh_phase5_confirmed_result_does_not_overclaim_failed_cleanup():
    ...
    services, _ = _phase5_result_services(session=session, results=(SimpleNamespace(detection_status=DetectionStatus.HIGH_CONFIDENCE, sha256=sha256),), incident=SimpleNamespace(id="internal-incident", sha256=sha256, status=IncidentStatus.OPEN), verification=SimpleNamespace(status=VerificationStatus.FAILED))
    controller = AutoGuardUIController(services, UIMessageBus())
    try: result = controller._scan_result_snapshot((SimpleNamespace(session_id=session.id),))
    finally: controller.shutdown()
...
def test_ux_refresh_phase5_scan_result_component_and_page_contract():
    ...
    assert all(req in component for req in ("class ScanResultPanel", "No threats detected in the files successfully scanned.", "Scan complete", "Review files", "View threat"))
    for forbidden in ("LOW_CONFIDENCE", "HIGH_CONFIDENCE", "incident_id", "session_id", "sha256"): assert forbidden not in component
    assert all(req in page for req in ("ScanResultPanel", "prepare_scan_result", 'message.kind == "scan_result_ready"', 'navigate_to("activity")', 'navigate_to("threats")'))
...
def _phase6_incident(status, *, name="sample.bin", matching_paths=(), location_count=1):
    ...
    incident = SimpleNamespace(id=f"ref-{status.value.lower()}", status=status, first_observed_path=rf"c:\users\fonti\downloads\{name}", last_observed_at=when)
    files = tuple(SimpleNamespace(path=rf"c:\test\location-{index}\{name}") for index in range(location_count))
    events = tuple(SimpleNamespace(event_type=IncidentEventType.MATCHING_COPY_FOUND, path=path) for path in matching_paths)
...
def test_ux_refresh_phase6_threat_read_model_translates_statuses_and_sections():
    configured = [_phase6_incident(s) for s in (IncidentStatus.OPEN, IncidentStatus.INVESTIGATING, IncidentStatus.CONTAINED, IncidentStatus.REAPPEARED, IncidentStatus.RESTORED, IncidentStatus.RESOLVED)]
...
def test_ux_refresh_phase6_threat_read_model_uses_location_and_matching_copy_evidence():
    ...
    assert row["name"] == "test-threat.bin" and row["location"] == "3 observed locations" and row["matching_copies"] == 2 and row["action"] == "View details"
...
def test_ux_refresh_phase6_threats_page_is_user_facing_and_service_driven():
    ...
    controller_source = (Path(__file__).resolve().parents[1] / "app/ui/controller.py").read_text(encoding="utf-8")
    assert all(req in source for req in ('title = "Threats"', "Needs attention", "Contained", "Resolved", "No active threats", "open_threat_details"))
    ...
    assert all(req in controller_source for req in ("def load_threats", "self.services.incidents.list_incidents", "self.services.incidents.get_incident"))