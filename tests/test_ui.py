...
class FakeScanner:
    ...
    def count_scan_entries(self, path, **kwargs): return 1
...
def test_controller_dispatches_manual_scan_off_calling_thread():
    bus, scanner = UIMessageBus(), FakeScanner()
    controller = AutoGuardUIController(fake_services(scanner), bus)
    calling_thread, started = threading.get_ident(), time.monotonic()
    ...
    try:
        ...
        assert any(item.kind == "scan_total" and item.payload["total"] == 1 for item in seen)
        progress = next(item for item in seen if item.kind == "scan_result")
        assert progress.payload["processed"] == 1; assert progress.payload["total"] == 1; assert progress.payload["progress"] == 1.0; assert message.payload["result"].session_id == "session-1"
...
def test_controller_emits_task_failure_without_raising_on_ui_thread():
    ...
    try: assert "PermissionError" in failure.payload["error"]; assert "simulated locked target" in failure.payload["error"]
    finally: controller.shutdown()
...
def test_scan_progress_callback_failure_does_not_change_scanner_result(tmp_path):
    ...
    scanner = Scanner(Detector(SignatureStore()), ThreatTrail(database)); target = tmp_path / "safe.txt"; target.write_text("harmless", encoding="utf-8")
...
def test_scanner_count_scan_entries_matches_emitted_results(tmp_path):
    database = Database(tmp_path / "autoguard.db"); database.initialize()
    ...
    total = scanner.count_scan_entries(folder); summary = scanner.scan(folder)
    assert total == 2; assert len(summary.results) == total