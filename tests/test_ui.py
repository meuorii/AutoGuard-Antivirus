...
def test_controller_dispatches_manual_scan_off_calling_thread():
    bus, scanner = UIMessageBus(), FakeScanner(); controller = AutoGuardUIController(fake_services(scanner), bus); calling_thread, started = threading.get_ident(), time.monotonic()
...
def test_controller_never_runs_metadata_precount_before_scan():
    bus, scanner = UIMessageBus(), FakeScanner(); controller = AutoGuardUIController(fake_services(scanner), bus); controller.start_scan("instant.txt"); _, seen = wait_for(bus, "scan_completed")
    try:
...
def _home_services(*, file_running=True, usb_running=True, scheduler_running=True, quick_running=False, full_running=False, incidents=(), sessions=(), details=None, quarantined=0):
    health = lambda r: SimpleNamespace(running=r, status=SimpleNamespace(value="RUNNING" if r else "STOPPED"))
    class History:
...
def test_ux_refresh_phase2_home_navigation_and_quick_scan_are_presentation_wired():
    source = (Path(__file__).resolve().parents[1] / "app/ui/dashboard.py").read_text(encoding="utf-8"); strings = {node.value for node in ast.walk(ast.parse(source)) if isinstance(node, ast.Constant) and isinstance(node.value, str)}
    assert {"Last Scan", "Threats", "Quarantine", "Protection", "Recent Activity", "Run Quick Scan", "View activity"} <= strings and "start_quick_scan" in source and 'navigate_to("activity")' in source
...
def test_ux_refresh_phase3_custom_scan_delegates_to_controller_without_scanner_logic():
    source = (Path(__file__).resolve().parents[1] / "app/ui/scan_page.py").read_text(encoding="utf-8")
    assert {"start_scan", "start_quick_scan", "start_full_scan"} <= {node.attr for node in ast.walk(ast.parse(source)) if isinstance(node, ast.Attribute) and isinstance(node.value, ast.Attribute) and isinstance(node.value.value, ast.Name) and node.value.value.id == "self" and node.value.attr == "controller"}
...
def test_ux_refresh_phase4_active_scan_is_focused_and_has_no_fake_pause():
    ...
    for req in ("Files checked", "Threats found", "Currently checking", "More details", "Discovered", "Processed", "Skipped", "Errors", "Suspicious", "Dangerous", "Stop Scan"): assert req in source
...
def test_ux_refresh_phase4_stop_scan_uses_cooperative_interrupt_without_blocking_ui():
    ...
    class InterruptibleScanner(FakeScanner):
        def __init__(self): super().__init__(); self.worker_started, self.received_interrupt_check = threading.Event(), False
        def scan(self, path, source, *, scan_type, interrupt_check=None, on_result=None, on_discovered=None):
            self.thread_id, self.received_interrupt_check = threading.get_ident(), callable(interrupt_check); self.worker_started.set()
            if on_discovered: on_discovered(str(path), "file")
    ...
    bus, scanner = UIMessageBus(), InterruptibleScanner(); controller = AutoGuardUIController(fake_services(scanner), bus); calling_thread = threading.get_ident()
    try:
        ...
        assert not any(m.kind in ("task_failed", "scan_completed") for m in seen)
    finally: controller.shutdown()
...
def _phase5_result_services(*, session, results=(), incident=None, incident_events=(), verification=None):
    class History:
        ...
        def get_scan(self, session_id):
            self.thread_id = threading.get_ident(); return SimpleNamespace(session=session, results=tuple(results)) if session_id == session.id else None
    history = History()
    class Incidents:
...
def test_ux_refresh_phase5_confirmed_result_claims_handled_only_with_verified_containment():
    ...
    session = _phase5_session(dangerous=1); sha256 = "a" * 64; stored = SimpleNamespace(detection_status=DetectionStatus.HIGH_CONFIDENCE, sha256=sha256); incident = SimpleNamespace(id="internal-incident", sha256=sha256, status=IncidentStatus.CONTAINED); event = SimpleNamespace(event_type=IncidentEventType.MATCHING_COPY_FOUND, occurred_at=session.started_at + timedelta(seconds=4))
    services, _ = _phase5_result_services(session=session, results=(stored,), incident=incident, incident_events=(event, event), verification=SimpleNamespace(status=VerificationStatus.VERIFIED)); controller = AutoGuardUIController(services, UIMessageBus())
    try: result = controller._scan_result_snapshot((SimpleNamespace(session_id=session.id),))
    finally: controller.shutdown()
...
def test_ux_refresh_phase5_confirmed_result_does_not_overclaim_failed_cleanup():
    ...
    from app.incidents import IncidentStatus
    from app.models import DetectionStatus
    session = _phase5_session(dangerous=1); sha256 = "b" * 64; stored = SimpleNamespace(detection_status=DetectionStatus.HIGH_CONFIDENCE, sha256=sha256); incident = SimpleNamespace(id="internal-incident", sha256=sha256, status=IncidentStatus.OPEN)
    services, _ = _phase5_result_services(session=session, results=(stored,), incident=incident, verification=SimpleNamespace(status=VerificationStatus.FAILED)); controller = AutoGuardUIController(services, UIMessageBus())
    try: result = controller._scan_result_snapshot((SimpleNamespace(session_id=session.id),))
    finally: controller.shutdown()
...
def test_ux_refresh_phase5_scan_result_component_and_page_contract():
    ...
    for req in ("class ScanResultPanel", "No threats detected in the files successfully scanned.", "Scan complete", "Review files", "View threat"): assert req in component
    for forbidden in ("LOW_CONFIDENCE", "HIGH_CONFIDENCE", "incident_id", "session_id", "sha256"): assert forbidden not in component
    for req in ("ScanResultPanel", "prepare_scan_result", 'message.kind == "scan_result_ready"', 'navigate_to("activity")', 'navigate_to("threats")'): assert req in page
...
def _phase6_incident(status, *, name="sample.bin", matching_paths=(), location_count=1):
    ...
    files = tuple(SimpleNamespace(path=rf"c:\test\location-{index}\{name}") for index in range(location_count)); events = tuple(SimpleNamespace(event_type=IncidentEventType.MATCHING_COPY_FOUND, path=path) for path in matching_paths)
...
def test_ux_refresh_phase6_threat_read_model_translates_statuses_and_sections():
    ...
    class Incidents:
        def list_incidents(self, limit): assert limit == 200; return [item[0] for item in configured]
        def get_incident(self, threat_ref): return details_by_id.get(threat_ref)
    controller = AutoGuardUIController(SimpleNamespace(incidents=Incidents()), UIMessageBus())
    try: rows = controller._threat_rows()
    finally: controller.shutdown()
    assert [(row["status"], row["section"]) for row in rows] == [("Needs attention", "Needs attention"), ("Being reviewed", "Needs attention"), ("Contained", "Contained"), ("Detected again", "Needs attention"), ("Restored", "Needs attention"), ("Resolved", "Resolved")]
    rendered = str(rows)
    for raw in ("OPEN", "INVESTIGATING", "CONTAINED", "REAPPEARED", "RESTORED", "RESOLVED"): assert raw not in rendered
...
def test_ux_refresh_phase6_threat_read_model_uses_location_and_matching_copy_evidence():
    ...
    class Incidents:
        def list_incidents(self, limit): return [incident]
        def get_incident(self, threat_ref): return details
    controller = AutoGuardUIController(SimpleNamespace(incidents=Incidents()), UIMessageBus())
...
def test_ux_refresh_phase6_open_threat_details_prepares_future_navigation():
    ...
    assert controller.selected_threat_ref == "internal-reference" and [m.kind for m in messages] == ["threat_details_requested", "navigate"] and messages[-1].payload == {"page": "threats"}
...
def test_ux_refresh_phase6_threats_page_is_user_facing_and_service_driven():
    ...
    for req in ('title = "Threats"', "Needs attention", "Contained", "Resolved", "No active threats", "open_threat_details"): assert req in source
    for forbidden in ("sha256", "incident_id", "event_type.value", "detection_rule", "SELECT * FROM threat_incidents"): assert forbidden not in source
...
def _phase7_services(*, status=None, verification_status=None):
    ...
    status = status or IncidentStatus.CONTAINED; when = datetime(2026, 9, 17, 1, 10, tzinfo=timezone.utc); incident = SimpleNamespace(id="internal-phase7-incident", sha256="a" * 64, status=status, first_observed_path=r"c:\users\fonti\downloads\test-threat.bin", last_observed_at=when)