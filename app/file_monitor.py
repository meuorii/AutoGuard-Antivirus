...
@dataclass(frozen=True)
class MonitorHealth:
    status: MonitorStatus; running: bool; observer_alive: bool; workers_alive: int; monitored_paths: tuple[str, ...]
    scheduled_paths: tuple[str, ...]; queue_depth: int; events_received: int; events_queued: int; events_debounced: int
    scans_dispatched: int; stability_failures: int; scan_failures: int; last_error: str | None
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path) as key:
            ...
            for name in (downloads_guid, "Desktop", "Personal"):
                raw, _ = winreg.QueryValueEx(key, name); values.append(normalize_path(os.path.expandvars(str(raw))))
        return tuple(values)  # type: ignore[return-value]
    except (OSError, ImportError, TypeError, ValueError): return None
...
class _WatchdogHandler(FileSystemEventHandler):
    def __init__(self, monitor: "FileMonitor") -> None: super().__init__(); self.monitor = monitor
...
class FileMonitor:
    ...
    def __init__(
        self, scanner: Scanner, monitored_paths: Iterable[str | Path] | None = None, *, ignored_paths: Iterable[str | Path] = (),
        debounce_seconds: float = 1.0, stability_period_seconds: float = 1.0, stability_poll_seconds: float = 0.25,
        stability_timeout_seconds: float = 30.0, worker_count: int = 1, observer_factory: Callable[[], ObserverProtocol] | None = None,
        clock: Callable[[], float] = time.monotonic, sleep: Callable[[float], None] = time.sleep, stat_func: Callable[[Path], os.stat_result] | None = None,
    ) -> None:
        ...
        self.scanner = scanner; roots = default_monitored_paths() if monitored_paths is None else tuple(monitored_paths)
        self._monitored_paths = self._deduplicate_roots(roots); self._ignored_paths = self._deduplicate_roots(ignored_paths)
        self.debounce_seconds = float(debounce_seconds); self.stability_period_seconds = float(stability_period_seconds)
        self.stability_poll_seconds = float(stability_poll_seconds); self.stability_timeout_seconds = float(stability_timeout_seconds)
        self.worker_count = worker_count; self._observer_factory = observer_factory; self._clock = clock; self._sleep = sleep
        self._stat = stat_func if stat_func is not None else lambda path: path.stat()

        self._queue: queue.Queue[_ScanRequest | object] = queue.Queue(); self._lock = threading.Lock()
        self._scheduled: set[Path] = set(); self._last_completed: dict[Path, float] = {}; self._observer: ObserverProtocol | None = None
        self._handler = _WatchdogHandler(self); self._workers: list[threading.Thread] = []; self._running = False
        self._scheduled_roots: tuple[Path, ...] = (); self._last_error: str | None = None; self._events_received = 0
        self._events_queued = 0; self._events_debounced = 0; self._scans_dispatched = 0; self._stability_failures = 0; self._scan_failures = 0
    ...
    def health(self) -> MonitorHealth:
        ...
        with self._lock:
            running = self._running; last_error = self._last_error
            counters = (self._events_received, self._events_queued, self._events_debounced, self._scans_dispatched, self._stability_failures, self._scan_failures)
        ...
        return MonitorHealth(
            status=status, running=running, observer_alive=observer_alive, workers_alive=workers_alive,
            monitored_paths=tuple(str(path) for path in self._monitored_paths), scheduled_paths=tuple(str(path) for path in self._scheduled_roots),
            queue_depth=self._queue.qsize(), events_received=counters[0], events_queued=counters[1], events_debounced=counters[2],
            scans_dispatched=counters[3], stability_failures=counters[4], scan_failures=counters[5], last_error=last_error,
        )
    ...
    def start(self) -> MonitorHealth:
        ...
        with self._lock: already_running = self._running
        if already_running: return self.health()
        ...
        for root in self._monitored_paths:
            ...
            observer.schedule(self._handler, str(root), recursive=True); scheduled.append(root)
        if not scheduled: raise ValueError("None of the configured monitored directories currently exist.")

        self._observer = observer; self._scheduled_roots = tuple(scheduled)
        self._workers = [threading.Thread(target=self._worker_loop, name=f"AutoGuardFileMonitor-{index + 1}", daemon=True) for index in range(self.worker_count)]
        with self._lock: self._running = True; self._last_error = None
        ...
        try: observer.start()
            ...
            self._workers.clear(); self._observer = None; self._scheduled_roots = (); raise
        return self.health()
    ...
    def stop(self, timeout: float = 5.0) -> MonitorHealth:
        ...
        with self._lock: was_running = self._running; self._running = False
        ...
        self._workers.clear(); self._observer = None; self._scheduled_roots = ()
        return self.health()
    ...
    def handle_event(self, event_type: str, src_path: str | Path, *, dest_path: str | Path | None = None, is_directory: bool = False) -> bool:
        ...
        if event_type not in {"created", "modified", "moved"}: return False
    ...
    def queue_path(self, path: str | Path, *, event_type: str = "created") -> bool:
        ...
        with self._lock:
            ...
            if candidate in self._scheduled or (last_completed is not None and now - last_completed < self.debounce_seconds):
    ...
    def check_stability(self, path: str | Path) -> StabilityResult:
        ...
        candidate = normalize_path(path); started = self._clock(); stable_since: float | None = None; previous: tuple[int, int] | None = None
        ...
        while True:
            ...
            if mode is not None and not stat.S_ISREG(mode): return StabilityResult(False, "Path is not a regular file.")

            signature = (int(info.st_size), int(info.st_mtime_ns))
            if signature != previous: previous = signature; stable_since = now
            ...
            if now - started >= self.stability_timeout_seconds:
                return StabilityResult(False, "File did not become stable before the stability timeout.", *signature)
    ...
    def _worker_loop(self) -> None:
        while True:
            ...
            try:
                ...
                self._record_error(f"Monitor worker failed: {type(error).__name__}: {error}")
            finally: self._queue.task_done()
