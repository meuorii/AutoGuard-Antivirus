...
def _session(row: sqlite3.Row) -> ScanSession:
    return ScanSession(
        id=row["id"], scan_type=ScanType(row["scan_type"]), source_path=row["source_path"], source=ScanSource(row["source"]),
        started_at=datetime.fromisoformat(row["started_at"]), finished_at=datetime.fromisoformat(row["finished_at"]) if row["finished_at"] else None,
...
def _result(row: sqlite3.Row) -> StoredScanResult:
    return StoredScanResult(
        id=row["id"], session_id=row["session_id"], path=row["path"], sha256=row["sha256"], result_category=ScanStatus(row["result_category"]),
        entry_kind=row["entry_kind"], detection_status=DetectionStatus(row["detection_status"]) if row["detection_status"] else None,
        detection_confidence=row["detection_confidence"], rule_name=row["rule_name"], reason=row["reason"], detection_reason=row["detection_reason"],
        matched_signature_name=row["matched_signature_name"], matched_signature_kind=SignatureKind(row["matched_signature_kind"]) if row["matched_signature_kind"] else None,
...
class ScanHistory:
    ...
    def start_session(self, scan_type: ScanType, source_path: str | Path, source: ScanSource | None = None) -> ScanSession:
        scan_type, session_id, source_path = ScanType(scan_type), str(uuid4()), str(normalize_path(source_path))
        source = scan_type.default_source if source is None else ScanSource(source)
        with self.database.connection() as connection:
            connection.execute("INSERT INTO scan_sessions (id, scan_type, source_path, source, started_at) VALUES (?, ?, ?, ?, ?)", (session_id, scan_type.value, source_path, source.value, _utc_now()))
            return _session(connection.execute("SELECT * FROM scan_sessions WHERE id = ?", (session_id,)).fetchone())
    ...
    def record_result(self, session_id: str, result: ScanResult) -> StoredScanResult:
        ...
        hashes = {value for value in (result.sha256, detection.sha256 if detection else None, result.observation.sha256 if result.observation else None) if value is not None}
        ...
        with self.database.connection() as connection:
            ...
            cursor = connection.execute(
                """INSERT INTO scan_results (
                    session_id, path, sha256, result_category, entry_kind, detection_status, detection_confidence,
                    rule_name, reason, detection_reason, matched_signature_name, matched_signature_kind, recorded_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (session_id, str(normalize_path(result.path)), sha256, result.status.value, result.kind, detection.status.value if detection else None,
                 detection.confidence if detection else None, detection.rule_name if detection else None, result.reason, detection.reason if detection else None,
                 signature.name if signature else None, signature.kind.value if signature else None, _utc_now()),
            )
            connection.execute(f"UPDATE scan_sessions SET {', '.join(f'{name} = {name} + ?' for name in COUNTER_NAMES)} WHERE id = ?", (*[delta[name] for name in COUNTER_NAMES], session_id))
            return _result(connection.execute("SELECT * FROM scan_results WHERE id = ?", (cursor.lastrowid,)).fetchone())
    ...
    def mark_interrupted(self, session_id: str, *, reason: str) -> ScanSession:
        ...
        with self.database.connection() as connection:
            connection.execute("BEGIN IMMEDIATE"); row = connection.execute("SELECT * FROM scan_sessions WHERE id = ?", (session_id,)).fetchone()
            if row is None: raise KeyError(f"Unknown scan session: {session_id}")
            if row["scan_type"] != ScanType.USB.value: raise ValueError("Only USB scan sessions may be externally marked interrupted.")
            error = reason if not row["error"] else f"{row['error']}; {reason}"
            finished_at = row["finished_at"] or max(_utc_now(), row["started_at"])
            connection.execute("UPDATE scan_sessions SET status = ?, finished_at = ?, error = ? WHERE id = ?", (ScanSessionStatus.INCOMPLETE.value, finished_at, error, session_id))
            return _session(connection.execute("SELECT * FROM scan_sessions WHERE id = ?", (session_id,)).fetchone())
    ...
    def recent_scans(self, limit: int = 20) -> list[ScanSession]:
        ...
        with self.database.connection() as connection:
            return [_session(row) for row in connection.execute("SELECT * FROM scan_sessions ORDER BY started_at DESC, rowid DESC LIMIT ?", (limit,)).fetchall()]
