...
class IncidentStatus(str, Enum): OPEN = "OPEN"; INVESTIGATING = "INVESTIGATING"; CONTAINED = "CONTAINED"; REAPPEARED = "REAPPEARED"; RESTORED = "RESTORED"; RESOLVED = "RESOLVED"
class IncidentEventType(str, Enum): FIRST_OBSERVED = "FIRST_OBSERVED"; MATCHING_LOCATION_OBSERVED = "MATCHING_LOCATION_OBSERVED"; REPEATED_DETECTION = "REPEATED_DETECTION"; STATUS_CHANGED = "STATUS_CHANGED"

@dataclass(frozen=True)
class ThreatIncident: id: str; sha256: str; status: IncidentStatus; first_observed_path: str; created_at: datetime; updated_at: datetime; last_observed_at: datetime; detection_count: int

@dataclass(frozen=True)
class IncidentFile: id: int; incident_id: str; path: str; first_seen: datetime; last_seen: datetime; seen_count: int; first_source: ScanSource; last_source: ScanSource; first_detection_reason: str; last_detection_reason: str

@dataclass(frozen=True)
class IncidentEvent: id: int; incident_id: str; event_type: IncidentEventType; occurred_at: datetime; path: str | None; source: ScanSource | None; detection_reason: str | None; old_status: IncidentStatus | None; new_status: IncidentStatus | None

@dataclass(frozen=True)
class IncidentDetails: incident: ThreatIncident; files: tuple[IncidentFile, ...]; events: tuple[IncidentEvent, ...]
...
class IncidentService:
    ...
    def create_incident(self, sha256: str, path: str | Path, source: ScanSource, detection_reason: str) -> ThreatIncident:
        ...
        sha256, path, source, incident_id, now = _sha256(sha256), str(normalize_path(path)), ScanSource(source), str(uuid4()), _utc_now()
