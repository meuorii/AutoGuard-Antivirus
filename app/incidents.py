...
class IncidentEventType(str, Enum):
    ...
    MATCHING_CLEANUP_STARTED, MATCHING_COPY_FOUND, MATCHING_COPY_QUARANTINED, MATCHING_COPY_ALREADY_CONTAINED = "MATCHING_CLEANUP_STARTED", "MATCHING_COPY_FOUND", "MATCHING_COPY_QUARANTINED", "MATCHING_COPY_ALREADY_CONTAINED"
    MATCHING_CLEANUP_FAILED, MATCHING_CLEANUP_FINISHED = "MATCHING_CLEANUP_FAILED", "MATCHING_CLEANUP_FINISHED"
...
class IncidentService:
    ...
    def create_incident(self, sha256: str, path: str | Path, source: ScanSource, detection_reason: str) -> ThreatIncident:
        ...
        with self.database.connection() as conn:
            ...
            conn.execute("INSERT INTO threat_incidents (id, sha256, status, first_observed_path, created_at, updated_at, last_observed_at, detection_count) VALUES (?, ?, ?, ?, ?, ?, ?, 1)", (incident_id, sha256, IncidentStatus.OPEN.value, path, now, now, now))
            self._insert_file(conn, incident_id, path, source, detection_reason, now)
            self._insert_event(conn, incident_id, IncidentEventType.FIRST_OBSERVED, now, path=path, source=source, detection_reason=detection_reason)
    ...
    def attach_matching_file(self, incident_id: str, path: str | Path, source: ScanSource, detection_reason: str) -> IncidentFile:
        ...
        with self.database.connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            incident = self._require_active(conn, incident_id)
            existing = conn.execute("SELECT * FROM incident_files WHERE incident_id = ? AND path = ?", (incident_id, path)).fetchone()
            ...
            self._upsert_file(conn, incident_id, path, source, detection_reason, now)
            self._mark_detection(conn, incident, now, path, source, detection_reason)
            self._insert_event(conn, incident_id, event_type, now, path=path, source=source, detection_reason=detection_reason)
            row = conn.execute("SELECT * FROM incident_files WHERE incident_id = ? AND path = ?", (incident_id, path)).fetchone()
        return _file(row)
    ...
    def append_event(self, incident_id: str, event_type: IncidentEventType, *, path: str | Path | None = None, source: ScanSource | None = None, detection_reason: str | None = None, old_status: IncidentStatus | None = None, new_status: IncidentStatus | None = None, reappearance_count: int | None = None) -> IncidentEvent:
        ...
        with self.database.connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            self._require_incident(conn, incident_id)
            cursor = self._insert_event(conn, incident_id, event_type, now, path=normalized_path, source=normalized_source, detection_reason=detection_reason, old_status=old, new_status=new, reappearance_count=reappearance_count)
            row = conn.execute("SELECT * FROM incident_events WHERE id = ?", (cursor.lastrowid,)).fetchone()
        return _event(row)
    ...
    def get_incident(self, incident_id: str) -> IncidentDetails | None:
        with self.database.connection() as conn:
            ...
            files = conn.execute("SELECT * FROM incident_files WHERE incident_id = ? ORDER BY first_seen, id", (incident_id,)).fetchall()
            events = conn.execute("SELECT * FROM incident_events WHERE incident_id = ? ORDER BY occurred_at, id", (incident_id,)).fetchall()
        return IncidentDetails(incident=_incident(row), files=tuple(_file(f) for f in files), events=tuple(_event(e) for e in events))
    ...
    def list_incidents(self, limit: int = 200) -> list[ThreatIncident]:
        ...
        with self.database.connection() as conn:
            rows = conn.execute("SELECT * FROM threat_incidents ORDER BY last_observed_at DESC, rowid DESC LIMIT ?", (limit,)).fetchall()
        return [_incident(r) for r in rows]

    def get_active_incidents(self) -> list[ThreatIncident]:
        with self.database.connection() as conn:
            rows = conn.execute("SELECT * FROM threat_incidents WHERE status <> ? ORDER BY last_observed_at DESC, rowid DESC", (IncidentStatus.RESOLVED.value,)).fetchall()
        return [_incident(r) for r in rows]
    ...
    def change_status(self, incident_id: str, status: IncidentStatus, *, reason: str | None = None) -> ThreatIncident:
        ...
        with self.database.connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            row = self._require_incident(conn, incident_id)
            old = IncidentStatus(row["status"])
            if old is status: return _incident(row)
            conn.execute("UPDATE threat_incidents SET status = ?, updated_at = ? WHERE id = ?", (status.value, now, incident_id))
            self._insert_event(conn, incident_id, IncidentEventType.STATUS_CHANGED, now, detection_reason=reason, old_status=old, new_status=status)
            row = conn.execute("SELECT * FROM threat_incidents WHERE id = ?", (incident_id,)).fetchone()
        return _incident(row)
    ...
    def record_high_confidence_detection(self, detection: DetectionResult, source: ScanSource) -> ThreatIncident:
        ...
        sha256, path, source, reason, now = _sha256(detection.sha256), str(normalize_path(detection.path)), ScanSource(source), detection.reason, _utc_now()
        if not reason: raise ValueError("High-confidence detection reason must not be empty.")
        with self.database.connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            row = conn.execute("SELECT * FROM threat_incidents WHERE sha256 = ? AND status <> ? ORDER BY created_at DESC, rowid DESC LIMIT 1", (sha256, IncidentStatus.RESOLVED.value)).fetchone()
            if row is None:
                incident_id = str(uuid4())
                conn.execute("INSERT INTO threat_incidents (id, sha256, status, first_observed_path, created_at, updated_at, last_observed_at, detection_count) VALUES (?, ?, ?, ?, ?, ?, ?, 1)", (incident_id, sha256, IncidentStatus.OPEN.value, path, now, now, now))
                self._insert_file(conn, incident_id, path, source, reason, now)
                self._insert_event(conn, incident_id, IncidentEventType.FIRST_OBSERVED, now, path=path, source=source, detection_reason=reason)
            else:
                incident_id = row["id"]
                existing_file = conn.execute("SELECT id FROM incident_files WHERE incident_id = ? AND path = ?", (incident_id, path)).fetchone()
                ...
                self._upsert_file(conn, incident_id, path, source, reason, now)
                self._mark_detection(conn, row, now, path, source, reason)
                self._insert_event(conn, incident_id, event_type, now, path=path, source=source, detection_reason=reason)
            current = conn.execute("SELECT * FROM threat_incidents WHERE id = ?", (incident_id,)).fetchone()
    ...
    def record_reappearance(self, incident_id: str, detection: DetectionResult, source: ScanSource, *, explanation: str) -> tuple[ThreatIncident, IncidentEvent]:
        ...
        with self.database.connection() as conn:
            ...
            self._upsert_file(conn, incident_id, path, source, detection.reason, now)
            conn.execute("UPDATE threat_incidents SET status = ?, updated_at = ?, last_observed_at = ?, detection_count = detection_count + 1, reappearance_count = ? WHERE id = ?", (IncidentStatus.REAPPEARED.value, now, now, count, incident_id))
            if old is not IncidentStatus.REAPPEARED:
                self._insert_event(conn, incident_id, IncidentEventType.STATUS_CHANGED, now, detection_reason=f"Exact SHA-256 reappearance detected; containment is pending. {explanation}", old_status=old, new_status=IncidentStatus.REAPPEARED)
            event_cursor = self._insert_event(conn, incident_id, IncidentEventType.REAPPEARANCE, now, path=path, source=source, detection_reason=explanation, reappearance_count=count)
            incident_row = conn.execute("SELECT * FROM threat_incidents WHERE id = ?", (incident_id,)).fetchone()
            event_row = conn.execute("SELECT * FROM incident_events WHERE id = ?", (event_cursor.lastrowid,)).fetchone()
