...
def _observation(row: sqlite3.Row) -> FileObservation:
    return FileObservation(
        id=row["id"], sha256=row["sha256"], path=row["path"],
...
class ThreatTrail:
    ...
    def record_fingerprint(self, fingerprint: FileFingerprint, source: ScanSource = ScanSource.MANUAL) -> FileObservation:
        ...
        with self.database.connection() as connection:
            ...
            connection.execute(
                ...
                """,
                (fingerprint.sha256, fingerprint.path, source.value, fingerprint.size_bytes, observed_at, observed_at),
            )
            row = connection.execute("SELECT * FROM file_observations WHERE sha256 = ? AND path = ?", (fingerprint.sha256, fingerprint.path)).fetchone()
            if row is None: raise RuntimeError("Observation was not persisted.")
            connection.execute(
                "INSERT INTO observation_events (observation_id, source, observed_at) VALUES (?, ?, ?)",
    ...
    def get_events(self, sha256: str) -> list[ObservationEvent]:
        ...
        return [
            ObservationEvent(
                id=row["id"], observation_id=row["observation_id"], sha256=row["sha256"],
                path=row["path"], source=ScanSource(row["source"]), size_bytes=row["size_bytes"],
