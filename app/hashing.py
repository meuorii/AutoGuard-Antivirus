...
class FileTooLargeError(OSError):
    """A file exceeds the configured byte limit, including during a read."""

class UnsupportedFileError(ValueError):
    """The supplied entry cannot be handled as a regular file."""
...
def inspect_file(path: str | Path, chunk_size: int = HASH_CHUNK_SIZE, *, preview_bytes: int = 0, max_size_bytes: int | None = None, follow_symlinks: bool = True) -> FileInspection:
    ...
    with open(normalized, "rb", opener=opener) as stream:
        ...
        while True:
            read_size = chunk_size if max_size_bytes is None else min(chunk_size, max_size_bytes - size_bytes + 1)
            if not (chunk := stream.read(read_size)): break
