...
class StartupSettings:
    ...
    usb_monitor_enabled: bool | None = None
...
class AutoGuardServices:
    config: AppConfig; database: Database; signatures: SignatureStore; incidents: IncidentService
    threat_trail: ThreatTrail; quarantine: QuarantineService; matching_cleanup: MatchingCopyCleanup
    cleanup_verifier: CleanupVerifier; reappearance: ReappearanceWatch; recovery: RecoveryService
    reports: ReportService; scanner: Scanner; file_monitor: FileMonitor | object | None
    usb_monitor: USBMonitor | object | None; scheduler: ScanScheduler
    startup_dispatch: ScanDispatch | None = None

@dataclass(frozen=True)
class ShutdownReport:
    scheduler_stopped: bool; usb_monitor_stopped: bool; file_monitor_stopped: bool
    errors: tuple[str, ...] = ()
...
class AutoGuardStartup:
    ...
    def initialize(self) -> AutoGuardServices:
        ...
        matching_cleanup = MatchingCopyCleanup(database, incidents, quarantine, trail, monitored_locations=monitor_paths)
        cleanup_verifier = CleanupVerifier(database, incidents, quarantine, monitored_locations=monitor_paths)
        reappearance = ReappearanceWatch(database, incidents); detector = Detector(signatures); recovery = RecoveryService(database, quarantine, detector, incidents); history = ScanHistory(database); reports = ReportService(history, incidents, quarantine, cleanup_verifier, recovery)
        scanner = Scanner(detector, trail, max_file_size_bytes=config.max_file_size_bytes, history=history, incidents=incidents, quarantine=quarantine, matching_cleanup=matching_cleanup, cleanup_verifier=cleanup_verifier, reappearance=reappearance)
        
        file_monitor = self.file_monitor_factory(scanner, monitor_paths, ignored_paths=(config.quarantine_dir, config.database_dir, config.logs_dir), debounce_seconds=self.settings.debounce_seconds, stability_period_seconds=self.settings.stability_seconds) if self.settings.file_monitor_enabled else None
        usb_monitor = self.usb_monitor_factory(scanner, poll_interval_seconds=self.settings.usb_poll_seconds) if self._usb_enabled() else None
        ...
        self.services = AutoGuardServices(config=config, database=database, signatures=signatures, incidents=incidents, threat_trail=trail, quarantine=quarantine, matching_cleanup=matching_cleanup, cleanup_verifier=cleanup_verifier, reappearance=reappearance, recovery=recovery, reports=reports, scanner=scanner, file_monitor=file_monitor, usb_monitor=usb_monitor, scheduler=scheduler)
    ...
    def _usb_enabled(self) -> bool: return os.name == "nt" if self.settings.usb_monitor_enabled is None else bool(self.settings.usb_monitor_enabled)
