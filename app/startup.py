...
class AutoGuardStartup:
    ...
    def __init__(self, config: AppConfig | None = None, settings: StartupSettings = StartupSettings(), *, signature_loader: Callable[[], SignatureStore] = load_signatures, file_monitor_factory: Callable[..., object] = FileMonitor, usb_monitor_factory: Callable[..., object] = USBMonitor, scheduler_factory: Callable[..., ScanScheduler] = ScanScheduler, apscheduler_factory: Callable[[], SchedulerBackend] | None = None) -> None:
        self.config, self.settings = config if config is not None else AppConfig(), settings
        self.signature_loader, self.file_monitor_factory = signature_loader, file_monitor_factory
        self.usb_monitor_factory, self.scheduler_factory, self.apscheduler_factory = usb_monitor_factory, scheduler_factory, apscheduler_factory
        self.services, self._started = None, False
    ...
    def initialize(self) -> AutoGuardServices:
        ...
        signatures, incidents, trail = self.signature_loader(), IncidentService(database), ThreatTrail(database)
        quarantine = QuarantineService(database, config.quarantine_dir, incidents)
        monitor_paths = self._configured_monitor_paths()
        quick_paths, full_paths = self._configured_quick_paths(monitor_paths), self._configured_full_paths(monitor_paths)
        ...
        scanner = Scanner(Detector(signatures), trail, max_file_size_bytes=config.max_file_size_bytes, incidents=incidents, quarantine=quarantine, matching_cleanup=matching_cleanup, cleanup_verifier=cleanup_verifier, reappearance=reappearance)
        ...
        self.services = AutoGuardServices(config=config, database=database, signatures=signatures, incidents=incidents, threat_trail=trail, quarantine=quarantine, matching_cleanup=matching_cleanup, cleanup_verifier=cleanup_verifier, reappearance=reappearance, scanner=scanner, file_monitor=file_monitor, usb_monitor=usb_monitor, scheduler=scheduler)
    ...
    def shutdown(self, *, timeout: float = 5.0) -> ShutdownReport:
        ...
        errors, scheduler_stopped, usb_stopped, file_stopped = [], True, True, True
