...
CREATE TABLE IF NOT EXISTS file_observations (
    ...
    source TEXT NOT NULL CHECK (source IN (
        'manual', 'startup', 'scheduled', 'file_monitor',
        'usb', 'matching_copy', 'recovery'
    )),
...
CREATE TABLE IF NOT EXISTS observation_events (
    ...
    observation_id INTEGER NOT NULL
        REFERENCES file_observations(id) ON DELETE RESTRICT,
    source TEXT NOT NULL CHECK (source IN (
        'manual', 'startup', 'scheduled', 'file_monitor',
        'usb', 'matching_copy', 'recovery'
    )),
...
CREATE INDEX IF NOT EXISTS idx_events_observation_time
    ON observation_events(observation_id, observed_at, id);

CREATE TABLE IF NOT EXISTS scan_sessions (
    id TEXT PRIMARY KEY NOT NULL,
    scan_type TEXT NOT NULL CHECK (scan_type IN (
        'manual', 'quick', 'full', 'startup', 'scheduled', 'usb',
        'real_time', 'matching_copy', 'recovery'
    )),
    ...
    CHECK (discovered_files = scanned_files + skipped_size + locked_files
           + unsupported_files + file_errors)
);
...
CREATE TABLE IF NOT EXISTS scan_results (
    ...
    result_category TEXT NOT NULL CHECK (result_category IN (
        'SCANNED', 'SKIPPED_SIZE', 'SKIPPED_LOCKED', 'SKIPPED_UNSUPPORTED', 'ERROR'
    )),
    entry_kind TEXT NOT NULL CHECK (entry_kind IN ('file', 'directory', 'unknown')),
    detection_status TEXT CHECK (
        detection_status IN ('NO_DETECTION', 'LOW_CONFIDENCE', 'HIGH_CONFIDENCE')
    ),
    ...
    matched_signature_kind TEXT CHECK (
        matched_signature_kind IN ('known_malicious', 'autoguard_test')
    ),
    recorded_at TEXT NOT NULL,
    CHECK (
        (detection_status IS NULL AND detection_confidence IS NULL
         AND rule_name IS NULL AND detection_reason IS NULL) OR
        (detection_status IS NOT NULL AND detection_confidence IS NOT NULL
         AND rule_name IS NOT NULL AND detection_reason IS NOT NULL AND sha256 IS NOT NULL)
    ),
    CHECK (
        (detection_status IS 'HIGH_CONFIDENCE' AND matched_signature_name IS NOT NULL
         AND matched_signature_kind IS NOT NULL) OR
        (detection_status IS NOT 'HIGH_CONFIDENCE' AND matched_signature_name IS NULL
         AND matched_signature_kind IS NULL)
    ),
    CHECK (result_category <> 'SCANNED' OR
           (entry_kind = 'file' AND sha256 IS NOT NULL AND detection_status IS NOT NULL))
);
...
CREATE TABLE IF NOT EXISTS incident_events (
    ...
    event_type TEXT NOT NULL CHECK (event_type IN (
        'FIRST_OBSERVED', 'MATCHING_LOCATION_OBSERVED',
        'REPEATED_DETECTION', 'STATUS_CHANGED',
        'QUARANTINE_STARTED', 'QUARANTINE_SUCCEEDED',
        'QUARANTINE_FAILED', 'QUARANTINE_INTEGRITY_FAILED'
    )),
    ...
    CHECK (
        (event_type = 'STATUS_CHANGED' AND old_status IS NOT NULL AND new_status IS NOT NULL)
        OR
        (event_type <> 'STATUS_CHANGED' AND path IS NOT NULL AND source IS NOT NULL
         AND detection_reason IS NOT NULL)
    )
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_active_incident_sha256
    ON threat_incidents(sha256) WHERE status <> 'RESOLVED';
CREATE INDEX IF NOT EXISTS idx_incident_files_incident
    ON incident_files(incident_id, first_seen, id);
...
CREATE TABLE IF NOT EXISTS quarantine_items (
    ...
    integrity_status TEXT NOT NULL CHECK (
        integrity_status IN ('PENDING', 'VERIFIED', 'FAILED')
    ),
    verified_sha256 TEXT CHECK (
        verified_sha256 IS NULL OR
        (length(verified_sha256) = 64 AND verified_sha256 NOT GLOB '*[^0-9a-f]*')
    ),
    ...
    CHECK (
        ...
        (integrity_status = 'VERIFIED' AND quarantined_at IS NOT NULL
         AND original_removed = 1 AND verified_sha256 = sha256)
    )
);

CREATE INDEX IF NOT EXISTS idx_quarantine_incident
    ON quarantine_items(incident_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS idx_quarantine_success_identity
    ON quarantine_items(incident_id, original_path, sha256)
...
def _migrate_incident_events_for_phase6(connection: sqlite3.Connection) -> None:
    ...
    connection.execute(
        """CREATE TABLE incident_events_phase6 (
            id INTEGER PRIMARY KEY,
            incident_id TEXT NOT NULL REFERENCES threat_incidents(id) ON DELETE RESTRICT,
            event_type TEXT NOT NULL CHECK (event_type IN (
                'FIRST_OBSERVED', 'MATCHING_LOCATION_OBSERVED',
    ...
    connection.execute(
        """INSERT INTO incident_events_phase6 (
               id, incident_id, event_type, occurred_at, path, source,
               detection_reason, old_status, new_status
           )
           SELECT id, incident_id, event_type, occurred_at, path, source,
                  detection_reason, old_status, new_status
