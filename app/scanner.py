...
ScanMode = Literal["auto", "file", "directory"]; DiscoveryCallback = Callable[[str, EntryKind], None]
...
class Scanner:
    def __init__(self, detector: Detector, threat_trail: ThreatTrail, *, max_file_size_bytes: int = DEFAULT_MAX_FILE_SIZE_BYTES, history: ScanHistory | None = None, incidents: IncidentService | None = None, quarantine: QuarantineService | None = None, matching_cleanup: MatchingCopyCleanup | None = None, cleanup_verifier: CleanupVerifier | None = None, reappearance: ReappearanceWatch | None = None) -> None:
        ...
        self.last_reappearance_results, self.last_matching_cleanup_results, self.last_cleanup_verification_results = (), (), ()
    ...
    def scan_file(self, path: str | Path, source: ScanSource | None = None, *, scan_type: ScanType | None = None, on_result: Callable[[ScanResult], None] | None = None, on_discovered: DiscoveryCallback | None = None) -> ScanResult:
        return self._run(path, source, scan_type, "file", on_result=on_result, on_discovered=on_discovered).results[0]

    def scan(self, path: str | Path, source: ScanSource | None = None, *, scan_type: ScanType | None = None, interrupt_check: Callable[[], bool] | None = None, on_result: Callable[[ScanResult], None] | None = None, on_discovered: DiscoveryCallback | None = None) -> ScanSummary:
        return self._run(path, source, scan_type, "auto", interrupt_check, on_result=on_result, on_discovered=on_discovered)

    def scan_directory(self, path: str | Path, source: ScanSource | None = None, *, scan_type: ScanType | None = None, interrupt_check: Callable[[], bool] | None = None, on_result: Callable[[ScanResult], None] | None = None, on_discovered: DiscoveryCallback | None = None) -> ScanSummary:
        return self._run(path, source, scan_type, "directory", interrupt_check, on_result=on_result, on_discovered=on_discovered)

    def _run(self, path: str | Path, source: ScanSource | None, scan_type: ScanType | None, mode: ScanMode, interrupt_check: Callable[[], bool] | None = None, *, on_result: Callable[[ScanResult], None] | None = None, on_discovered: DiscoveryCallback | None = None) -> ScanSummary:
        ...
        try:
            for result in self._iter_scan(root, source, mode, interrupt_check, on_discovered):
                result = replace(result, session_id=session.id); stage = f"saving result for {result.path}"; self.history.record_result(session.id, result); results.append(result)
