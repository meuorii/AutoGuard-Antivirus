...
from app.models import EntryKind, ScanResult, ScanSource, ScanStatus, ScanSummary, ScanType
...
class Scanner:
    def __init__(self, detector: Detector, threat_trail: ThreatTrail, *, max_file_size_bytes: int = DEFAULT_MAX_FILE_SIZE_BYTES, history: ScanHistory | None = None) -> None:
    ...
    def _iter_directory(self, root: Path, source: ScanSource) -> Iterator[ScanResult]:
        ...
        while pending:
            ...
            if info.st_ino and identity in visited:
                yield ScanResult(str(current), ScanStatus.SKIPPED_UNSUPPORTED, "Directory identity already visited; possible alias or loop.", "directory")
                continue
    ...
    def _process_file(self, path: Path, info: os.stat_result, source: ScanSource) -> ScanResult:
        ...
        detection = sha256 = None; stage = "read"
        try:
