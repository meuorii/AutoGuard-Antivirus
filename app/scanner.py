...
class Scanner:
    def __init__(
        self, detector: Detector, threat_trail: ThreatTrail, *, max_file_size_bytes: int = DEFAULT_MAX_FILE_SIZE_BYTES,
        history: ScanHistory | None = None, incidents: IncidentService | None = None, quarantine: QuarantineService | None = None,
        matching_cleanup: MatchingCopyCleanup | None = None, cleanup_verifier: CleanupVerifier | None = None, reappearance: ReappearanceWatch | None = None,
    ) -> None:
        ...
        self._database_files = {normalize_path(str(db.path) + s) for db in (threat_trail.database, self.history.database) for s in ("", "-wal", "-shm", "-journal")}
    ...
    def scan(self, path: str | Path, source: ScanSource | None = None, *, scan_type: ScanType | None = None, interrupt_check: Callable[[], bool] | None = None) -> ScanSummary:
        return self._run(path, source, scan_type, "auto", interrupt_check)

    def scan_directory(self, path: str | Path, source: ScanSource | None = None, *, scan_type: ScanType | None = None, interrupt_check: Callable[[], bool] | None = None) -> ScanSummary:
        return self._run(path, source, scan_type, "directory", interrupt_check)

    def _run(self, path: str | Path, source: ScanSource | None, scan_type: ScanType | None, mode: ScanMode, interrupt_check: Callable[[], bool] | None = None) -> ScanSummary:
        ...
        try:
            for result in self._iter_scan(root, source, mode, interrupt_check):
                ...
                stage = f"saving result for {result.path}"; self.history.record_result(session.id, result); results.append(result); stage = "processing"
        ...
        if self.matching_cleanup is not None:
            for i_id in incident_ids: cleanup_results.append(self.matching_cleanup.cleanup_incident(i_id))
        self.last_matching_cleanup_results = tuple(cleanup_results)
        if self.cleanup_verifier is not None:
            for i_id in incident_ids: verification_results.append(self.cleanup_verifier.verify_incident(i_id))
    ...
    def _iter_scan(self, root: Path, source: ScanSource, mode: ScanMode, interrupt_check: Callable[[], bool] | None = None) -> Iterator[ScanResult]:
        ...
        if mode == "directory" and not directory:
            yield ScanResult(str(root), ScanStatus.SKIPPED_UNSUPPORTED, "Expected a regular directory, not a file or link/reparse point.", "directory" if stat.S_ISDIR(info.st_mode) else "file")
    ...
    def _raise_if_interrupted(interrupt_check: Callable[[], bool] | None, root: Path) -> None:
        if interrupt_check is not None and interrupt_check():
            raise ScanInterruptedError(f"Scan interrupted because the scan source is no longer available: {root}")
    ...
    def _process_file(self, path: Path, info: os.stat_result, source: ScanSource) -> ScanResult:
        ...
        if is_link_or_reparse(info) or not stat.S_ISREG(info.st_mode):
            return ScanResult(str(path), ScanStatus.SKIPPED_UNSUPPORTED, "Only regular files are supported; links, reparse points, and special entries are skipped.", kind)
        if path in self._database_files:
            return ScanResult(str(path), ScanStatus.SKIPPED_UNSUPPORTED, "AutoGuard's active database files are excluded.")
        if self._quarantine_dir is not None and (path == self._quarantine_dir or self._quarantine_dir in path.parents):
            return ScanResult(str(path), ScanStatus.SKIPPED_UNSUPPORTED, "AutoGuard quarantine storage is excluded from scanning.")
        if info.st_size > self.max_file_size_bytes:
            return ScanResult(str(path), ScanStatus.SKIPPED_SIZE, f"Size {info.st_size} exceeds limit {self.max_file_size_bytes} bytes.")
        ...
        try:
            ...
            sha256 = inspection.fingerprint.sha256; stage = "detection"; detection = self.detector.detect_inspection(inspection)
            stage = "recording"; observation = self.threat_trail.record_fingerprint(inspection.fingerprint, source); result_reason = detection.reason
    ...
    def _failure(path: Path, error: Exception, kind: EntryKind) -> ScanResult:
        ...
        elif kind != "directory" and isinstance(error, OSError) and (isinstance(error, PermissionError) or error.errno in {errno.EACCES, errno.EPERM, errno.EBUSY, errno.EAGAIN} or getattr(error, "winerror", None) in {5, 32, 33}):
