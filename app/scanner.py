...
class ScanInterruptedError(RuntimeError):
    ...
    def __init__(self, message: str, session_id: str | None = None) -> None: self.session_id = session_id; super().__init__(message)
...
class Scanner:
    def __init__(
        ...
        matching_cleanup: MatchingCopyCleanup | None = None, cleanup_verifier: CleanupVerifier | None = None,
        ...
        self.detector = detector; self.threat_trail = threat_trail; self.max_file_size_bytes = max_file_size_bytes
        ...
        self.quarantine = quarantine; self.matching_cleanup = matching_cleanup; self.cleanup_verifier = cleanup_verifier
        self.reappearance = reappearance if reappearance is not None else ReappearanceWatch(threat_trail.database, self.incidents)
        self.last_reappearance_results: tuple[ReappearanceResult, ...] = (); self.last_matching_cleanup_results: tuple[MatchingCleanupResult, ...] = ()
        self.last_cleanup_verification_results: tuple[CleanupVerificationResult, ...] = (); self._current_reappearance_results: list[ReappearanceResult] = []
    ...
    def _run(self, path: str | Path, source: ScanSource | None, scan_type: ScanType | None, mode: ScanMode, interrupt_check: Callable[[], bool] | None = None, *, on_result: Callable[[ScanResult], None] | None = None) -> ScanSummary:
        if scan_type is None: source = ScanSource.MANUAL if source is None else ScanSource(source); scan_type = ScanType.from_source(source)
        else: scan_type = ScanType(scan_type); source = scan_type.default_source if source is None else ScanSource(source)
        ...
        try:
            for result in self._iter_scan(root, source, mode, interrupt_check):
                ...
                if on_result is not None:
                    ...
                    except Exception: pass  # UI/progress observers are advisory and must never turn a successful security scan into a scan failure.
                stage = "processing"
        ...
        # Matching-Copy Cleanup is deliberately deferred until the scan session is finalized.
        ...
        # Phase 8 verifies containment only after quarantine and any configured matching-copy cleanup have finished.
    ...
    def _process_file(self, path: Path, info: os.stat_result, source: ScanSource) -> ScanResult:
        ...
        detection = None; sha256 = None; stage = "read"
        try:
            ...
            sha256 = inspection.fingerprint.sha256; stage = "detection"
            detection = self.detector.detect_inspection(inspection); stage = "recording"
