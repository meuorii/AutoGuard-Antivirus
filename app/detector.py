...
class Detector:
    ...
    def detect_inspection(self, inspection: FileInspection) -> DetectionResult:
        ...
        if (signature := self.signatures.lookup(fingerprint.sha256)) is not None:
            ...
            return DetectionResult(status=DetectionStatus.HIGH_CONFIDENCE, confidence=1.0, reason=reason, rule_name="autoguard_test_signature" if is_test else "known_malicious_sha256", sha256=fingerprint.sha256, path=fingerprint.path, matched_signature=signature)
        ...
        if matches := [match for rule in self.rules if (match := rule(context)) is not None]:
            ...
            return DetectionResult(status=DetectionStatus.LOW_CONFIDENCE, confidence=strongest.confidence, reason=strongest.reason, rule_name=strongest.rule_name, sha256=fingerprint.sha256, path=fingerprint.path)

        reason = "No configured signature or heuristic matched this file."
        if is_script and fingerprint.size_bytes > SCRIPT_PREVIEW_BYTES:
            reason += f" Script pattern checks covered only the first {SCRIPT_PREVIEW_BYTES} bytes."
        return DetectionResult(status=DetectionStatus.NO_DETECTION, confidence=0.0, reason=reason, rule_name="no_rule_matched", sha256=fingerprint.sha256, path=fingerprint.path)