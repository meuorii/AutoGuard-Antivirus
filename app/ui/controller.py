...
class AutoGuardUIController:
    ...
    def _dashboard_snapshot(self) -> dict[str, Any]:
        ...
        recent = self.services.scanner.history.recent_scans(8)
        ...
        for session in recent:
            ...
            if len(recent_detections) >= 5: break
        return {
            "file_monitor": file_health, "usb_monitor": usb_health, "scheduler": scheduler_health,
            "quarantined_count": sum(1 for item in quarantine if item.state.value == "QUARANTINED"),
            "active_incidents": len(active_incidents), "recent_scans": recent, "recent_detections": recent_detections,
            "active_tasks": self.active_tasks(),
            "monitor_paths": tuple(str(path) for path in (self.services.file_monitor.monitored_paths() if self.services.file_monitor else ())),
        }
    ...
    def start_scan(self, path: str | Path, *, scan_type: ScanType = ScanType.MANUAL) -> None:
        ...
        def on_result(result: ScanResult) -> None:
            ...
            self.bus.publish("scan_result", path=result.path, status=result.status.value, detection=detection, reason=result.reason)
        def scan(): return self.services.scanner.scan(target, ScanSource.MANUAL, scan_type=scan_type, on_result=on_result)
    ...
    def _start_path_batch(self, label: str, paths: tuple[Path, ...], scan_type: ScanType) -> None:
        ...
        def on_result(result: ScanResult) -> None:
            self.bus.publish("scan_result", path=result.path, status=result.status.value, detection=result.detection.status.value if result.detection else None, reason=result.reason)
        def run_batch() -> list[Any]:
            summaries = []
            for path in paths:
                if path.exists(): summaries.append(self.services.scanner.scan(path, ScanSource.MANUAL, scan_type=scan_type, on_result=on_result))
            return summaries
