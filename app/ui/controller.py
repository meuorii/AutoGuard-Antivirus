...
class AutoGuardUIController:
    def __init__(self, services: AutoGuardServices, bus: UIMessageBus | None = None) -> None:
        ...
        self._active_tasks, self._scan_cancel_events, self._current_scan_task = set(), {}, None

    def shutdown(self) -> None: self.stop_scan(); self._executor.shutdown(wait=False, cancel_futures=False)
    ...
    def _display_name(path: str) -> str:
        if "\\" in path: return path.rstrip("\\").rsplit("\\", 1)[-1]
    ...
    def _dashboard_snapshot(self) -> dict[str, Any]:
        ...
        scheduler_health, quarantine, active_incidents = self.services.scheduler.health(), self.services.quarantine.list_quarantined_items(), self.services.incidents.get_active_incidents()
        ...
        if last_session is None: last_scan = {"value": "No scans yet", "detail": "Run a Quick Scan to get started"}
            ...
            label, when, checked = self._scan_label(last_session.scan_type), last_session.finished_at or last_session.started_at, int(last_session.counters.get("scanned_files", 0))
    ...
    def _friendly_scan_status(status: Any) -> str:
        ...
        return {"RUNNING": "In progress", "COMPLETED": "Completed", "INCOMPLETE": "Finished with some files skipped", "FAILED": "Could not finish"}.get(value, "Finished")
    ...
    def _scan_result_snapshot(self, summaries: tuple[Any, ...]) -> dict[str, Any]:
        ...
        details = [history.get_scan(str(sid)) for summary in summaries if (sid := getattr(summary, "session_id", None)) and history.get_scan(str(sid)) is not None]
        ...
        issue_note = ""
        if skipped or errors: issue_note = "Some files could not be checked. View scan details for more information."
        ...
        active_incidents = {inc.sha256: inc for inc in self.services.incidents.get_active_incidents()}
        contained = matching_copies = 0
        ...
        for sha256 in dangerous_hashes:
            ...
            if incident_details is not None: matching_copies += sum(1 for e in incident_details.events if e.event_type is IncidentEventType.MATCHING_COPY_FOUND and (started_at is None or e.occurred_at >= started_at))
        fully_contained = bool(dangerous_hashes) and contained == len(dangerous_hashes)
        if verification_states and all(s is VerificationStatus.VERIFIED for s in verification_states) and len(verification_states) == len(dangerous_hashes): cleanup_status = "Verified"
        elif any(s is VerificationStatus.FAILED for s in verification_states): cleanup_status = "Failed"
        elif any(s is VerificationStatus.PARTIAL for s in verification_states): cleanup_status = "Partial"
    ...
    def _incident_rows(self) -> list[dict[str, Any]]:
        with self.services.database.connection() as conn:
            ids = [row["id"] for row in conn.execute("SELECT id FROM threat_incidents ORDER BY last_observed_at DESC, rowid DESC LIMIT 200").fetchall()]
        rows: list[dict[str, Any]] = []
        for incident_id in ids:
            if (details := self.services.incidents.get_incident(incident_id)) is None: continue
    ...
    def load_threat_trail(self, limit: int = 200) -> None:
        def load() -> list[dict[str, Any]]:
            with self.services.database.connection() as conn:
                rows = conn.execute("SELECT e.id, o.sha256, o.path, e.source, e.observed_at, o.seen_count FROM observation_events e JOIN file_observations o ON o.id = e.observation_id ORDER BY e.observed_at DESC, e.id DESC LIMIT ?", (limit,)).fetchall()
    ...
    def start_scan(self, path: str | Path, *, scan_type: ScanType = ScanType.MANUAL) -> None:
        target = Path(path); task = f"scan:{target}"
        if (cancel_event := self._begin_ui_scan(task)) is None: return
        ...
        def scan():
            ...
            def on_discovered(path: str, kind: str) -> None:
                nonlocal discovered
                discovered += 1; self.bus.publish("scan_discovered", path=path, entry_kind=kind, discovered=discovered, processed=processed, task=task)
            def on_result(result: ScanResult) -> None:
                nonlocal discovered, processed
                processed += 1; discovered = max(discovered, processed)
    ...
    def _start_path_batch(self, label: str, paths: tuple[Path, ...], scan_type: ScanType) -> None:
        ...
        def run_batch() -> list[Any]:
            existing, discovered, processed, summaries = tuple(p for p in paths if p.exists()), 0, 0, []
            def on_discovered(path: str, kind: str) -> None:
                nonlocal discovered
                discovered += 1; self.bus.publish("scan_discovered", path=path, entry_kind=kind, discovered=discovered, processed=processed, task=task)
            def on_result(result: ScanResult) -> None:
