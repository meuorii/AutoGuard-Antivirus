...
class AutoGuardUIController:
    def __init__(self, services: AutoGuardServices, bus: UIMessageBus | None = None) -> None:
        self.services, self.bus, self._executor, self._lock = services, bus if bus is not None else UIMessageBus(), ThreadPoolExecutor(max_workers=4, thread_name_prefix="AutoGuardUI"), threading.RLock()
        self._active_tasks, self._scan_cancel_events, self._current_scan_task, self._selected_threat_ref = set(), {}, None, None
    ...
    def _submit(self, task_name: str, operation: Callable[[], Any], success_kind: str) -> None:
        ...
        def runner() -> None:
            ...
            except ScanInterruptedError as error: self.bus.publish("scan_stopped", task=task_name, session_id=error.session_id, reason="Scan stopped safely.")
    ...
    def _dashboard_snapshot(self) -> dict[str, Any]:
        ...
        scheduler_health, quarantine, active_incidents, recent = self.services.scheduler.health(), self.services.quarantine.list_quarantined_items(), self.services.incidents.get_active_incidents(), self.services.scanner.history.recent_scans(16)
        ...
        ui_scan_running = any(task == "quick-scan" or task == "full-scan" or task.startswith("scan:") for task in active_tasks)
        scan_running = ui_scan_running or bool(getattr(scheduler_health, "quick_scan_running", False) or getattr(scheduler_health, "full_scan_running", False))
    ...
    def _format_duration(seconds: float | None) -> str:
        ...
        hours, minutes = divmod(max(0, int(round(seconds))), 3600); minutes, secs = divmod(minutes, 60)
    ...
    def _scan_result_snapshot(self, summaries: tuple[Any, ...]) -> dict[str, Any]:
        history, details = self.services.scanner.history, []
        for summary in summaries:
            session_id = getattr(summary, "session_id", None)
            if session_id and (item := history.get_scan(str(session_id))) is not None: details.append(item)
        ...
        for item in details:
            for name in counter_names: counters[name] += int(item.session.counters.get(name, 0))
        if not details:
            for summary in summaries:
        ...
        scan_label = self._scan_label(scan_types[0]) if scan_types and all(value is scan_types[0] for value in scan_types) else "Scan"
        ...
        base = {"scan_type": scan_label, "duration": self._format_duration(duration_seconds), "files_checked": files_checked, "threats_found": dangerous, "skipped_files": skipped, "suspicious_files": suspicious}
        ...
        if dangerous == 0:
            ...
            return {**base, "kind": "suspicious", "title": "Review recommended", "message": f"AutoGuard found {suspicious:,} suspicious {noun}, but no confirmed threat was detected.", "note": "Suspicious evidence does not by itself confirm malware. " + issue_note}
        ...
        title, message = ("Threat handled", f"AutoGuard detected and contained {contained:,} confirmed {'threat' if contained == 1 else 'threats'}. Cleanup verification succeeded.") if fully_contained else ("Confirmed threat detected", "AutoGuard detected a confirmed threat, but containment or cleanup could not be fully verified. Review the threat details.")
    ...
    _THREAT_STATUS_LABELS = {
        IncidentStatus.OPEN: ("Needs attention", "Needs attention"), IncidentStatus.INVESTIGATING: ("Being reviewed", "Needs attention"),
        IncidentStatus.CONTAINED: ("Contained", "Contained"), IncidentStatus.REAPPEARED: ("Detected again", "Needs attention"),
        IncidentStatus.RESTORED: ("Restored", "Needs attention"), IncidentStatus.RESOLVED: ("Resolved", "Resolved")
    }
    ...
    def _threat_rows(self) -> list[dict[str, Any]]:
        ...
        for incident in self.services.incidents.list_incidents(200):
            ...
            matching_copy_count = len(matching_paths) if matching_paths else sum(1 for event in details.events if event.event_type is IncidentEventType.MATCHING_COPY_FOUND)

            rows.append({
                "threat_ref": incident.id, "section": section, "status": status_label,
                "status_key": {"Needs attention": "attention", "Being reviewed": "reviewing", "Contained": "contained", "Detected again": "reappeared", "Restored": "restored", "Resolved": "resolved"}[status_label],
                "name": self._display_name(incident.first_observed_path), "location": location_text, "detected": self._friendly_timestamp(incident.last_observed_at),
                "matching_copies": matching_copy_count, "action": "Review" if section == "Needs attention" else "View details"
            })
        return rows
    ...
    def close_threat_details(self) -> None:
        self._selected_threat_ref = None; self.bus.publish("threat_details_closed")

    def load_threat_details(self, threat_ref: str | None = None) -> None:
        if ref := str(threat_ref or self._selected_threat_ref or "").strip():
    ...
    def _threat_details_snapshot(self, threat_ref: str) -> dict[str, Any]:
        ...
        recovery_attempts = []
        for item in quarantine_items: recovery_attempts.extend(self.services.recovery.list_attempts(item.quarantine_id))
        ...
        matching_found = len(matching_found_paths) if matching_found_paths else sum(event.event_type is IncidentEventType.MATCHING_COPY_FOUND for event in details.events)
        ...
        matching_quarantined = len(matching_quarantined_paths) if matching_quarantined_paths else sum(event.event_type is IncidentEventType.MATCHING_COPY_QUARANTINED for event in details.events)
        cleanup_started = any(event.event_type is IncidentEventType.MATCHING_CLEANUP_STARTED for event in details.events)

        if detection is not None and detection.matched_signature_name: detection_text = f'AutoGuard detected this file because it matched the stored signature “{detection.matched_signature_name}”.'
        elif explanation is not None: detection_text = explanation.summary
        elif details.files and details.files[0].first_detection_reason: detection_text = "AutoGuard recorded confirmed threat evidence for this file."
        else: detection_text = "AutoGuard recorded a confirmed threat for this file."
        ...
        if matching_found:
            matching_text = f"AutoGuard found {matching_found:,} additional exact matching {'copy' if matching_found == 1 else 'copies'}."
            if matching_quarantined: matching_text += f" {matching_quarantined:,} matching {'copy was' if matching_quarantined == 1 else 'copies were'} quarantined."
        elif cleanup_started: matching_text = "Matching-Copy Cleanup ran and no additional exact matching copies were recorded."
        else: matching_text = "No Matching-Copy Cleanup discovery record is available for this threat."
        ...
        recovery_text = None
        if [attempt for attempt in recovery_attempts if attempt.status in (RecoveryStatus.RESTORED, RecoveryStatus.RESTORED_WITH_WARNING)]: recovery_text = "A user-requested restore was recorded for quarantined content related to this threat."
        elif incident.status is IncidentStatus.RESTORED: recovery_text = "This threat is recorded as restored after a user-requested recovery action."

        what_happened = [{"label": "Detection", "text": detection_text}, {"label": "Action taken", "text": action_text}, {"label": "Matching copies", "text": matching_text}, {"label": "Verification", "text": cleanup_text}]
        if recovery_text: what_happened.append({"label": "Recovery", "text": recovery_text})

        return {"threat_ref": incident.id, "name": self._display_name(incident.first_observed_path), "status": status_label, "status_key": status_key, "first_observed_path": incident.first_observed_path, "sha256": incident.sha256, "first_observed_at": self._friendly_timestamp(incident.first_observed_at), "last_observed_at": self._friendly_timestamp(incident.last_observed_at), "cleanup_status": cleanup_label, "what_happened": what_happened}