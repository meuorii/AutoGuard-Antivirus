...
NAV_ITEMS = (("home", "Home"), ("scan", "Scan"), ("threats", "Threats"), ("quarantine", "Quarantine"), ("activity", "Activity"), ("settings", "Settings"))
...
class Sidebar(ctk.CTkFrame):
    def __init__(self, master, on_navigate, **kwargs):
        ...
        self.grid_propagate(False); spacer_row = len(NAV_ITEMS) + 1; footer_row = spacer_row + 1; self.grid_rowconfigure(spacer_row, weight=1)
        ...
        footer = ctk.CTkFrame(self, fg_color=theme.SURFACE, border_width=1, border_color=theme.BORDER, corner_radius=10); footer.grid(row=footer_row, column=0, padx=12, pady=16, sticky="sew")
        ctk.CTkLabel(footer, text="Protection", text_color=theme.TEXT_DIM, font=(theme.FONT, 9)).pack(anchor="w", padx=12, pady=(10, 0))
        self.protection = ctk.CTkLabel(footer, text="●  Checking", text_color=theme.WARNING, font=(theme.FONT, 10, "bold")); self.protection.pack(anchor="w", padx=12, pady=(2, 10))
        self.set_active("home")

    def set_active(self, key: str) -> None:
        for name, button in self.buttons.items():
            button.configure(fg_color=theme.ACCENT_DARK if name == key else "transparent", text_color=theme.ACCENT if name == key else theme.TEXT_MUTED)
