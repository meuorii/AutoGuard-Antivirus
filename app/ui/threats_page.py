...
class ThreatsPage(BasePage):
    ...
    def __init__(self, master, controller, **kwargs):
        super().__init__(master, controller, **kwargs); self._loading = False; self._detail_ref = None; self._render_loading()

    def on_show(self) -> None:
        selected = self.controller.selected_threat_ref
        if selected: self._detail_ref = selected; self._render_details_loading(); self.controller.load_threat_details(selected); return
        self._detail_ref = None; self._loading = True; self._render_loading(); self.controller.load_threats()

    def handle_message(self, message) -> None:
        if message.kind == "threat_details_requested":
            self._detail_ref = message.payload.get("threat_ref"); self._render_details_loading() if self._detail_ref else None
        elif message.kind == "threat_details_data":
            if self._detail_ref: self._loading = False; self._render_details(message.payload.get("result", {}))
        elif message.kind == "threat_details_closed": self._detail_ref = None; self._loading = True; self._render_loading(); self.controller.load_threats()
        elif message.kind == "threats_data" and not self._detail_ref: self._loading = False; self._render(message.payload.get("result", ()))
        elif message.kind in {"incident_verified", "quarantine_deleted", "recovery_completed"}: self.controller.load_threat_details(self._detail_ref) if self._detail_ref else self.controller.load_threats()

    def _render_details_loading(self) -> None:
        self.clear_body(); self.body.grid_columnconfigure(0, weight=1); ctk.CTkLabel(self.body, text="Loading threat details…", font=(theme.FONT, 11), text_color=theme.TEXT_MUTED).grid(row=0, column=0, padx=12, pady=28, sticky="w")

    def _render_details(self, data) -> None:
        self.clear_body(); self.body.grid_columnconfigure(0, weight=1); ThreatDetailsView(self.body, data or {}, on_back=self.controller.close_threat_details).grid(row=0, column=0, sticky="ew")

    def _render_loading(self) -> None:
        self.clear_body(); self.body.grid_columnconfigure(0, weight=1); ctk.CTkLabel(self.body, text="Loading threats…", font=(theme.FONT, 11), text_color=theme.TEXT_MUTED).grid(row=0, column=0, padx=12, pady=28, sticky="w")
    ...
    def _render(self, rows) -> None:
        ...
        for row in rows:
            if row.get("section") in grouped: grouped[row.get("section")].append(row)
        ...
        for section in ("Contained", "Resolved"):
            if grouped[section]: next_row = self._render_section(next_row, section, grouped[section])
        if not any(grouped.values()): self._no_history(next_row)

    def _empty_attention(self, row: int) -> None:
        frame = ctk.CTkFrame(self.body, fg_color=theme.SURFACE, border_width=1, border_color=theme.BORDER, corner_radius=theme.RADIUS); frame.grid(row=row, column=0, padx=8, pady=(4, 12), sticky="ew"); frame.grid_columnconfigure(0, weight=1)
    ...
    def _threat_card(self, row: int, item) -> None:
        card = ctk.CTkFrame(self.body, fg_color=theme.SURFACE, border_width=1, border_color=theme.BORDER, corner_radius=theme.RADIUS); card.grid(row=row, column=0, padx=8, pady=5, sticky="ew"); card.grid_columnconfigure(0, weight=1)
        ...
        meta_parts = [f"Detected {item.get('detected', 'time unavailable')}"]; copies = int(item.get("matching_copies", 0) or 0)
