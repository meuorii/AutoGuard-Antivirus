"""Simple user-facing Home page for AutoGuard UX Refresh Phase 2.

The Home page presents existing protection and history data in everyday terms.
It does not own scanner logic, monitoring services, or persistence.
"""
...
_STATE_STYLE = {
    "protected": (theme.SUCCESS, theme.SUCCESS_DARK, "✓"),
    "scanning": (theme.ACCENT, theme.ACCENT_DARK, "↻"),
    "attention": (theme.WARNING, theme.WARNING_DARK, "!"),
    "issue": (theme.DANGER, theme.DANGER_DARK, "!"),
}
...
class _SummaryBlock(ctk.CTkFrame):
    def __init__(self, master, label: str):
        ...
        ctk.CTkLabel(
            ...
            anchor="w",
        ).grid(row=0, column=0, padx=16, pady=(14, 5), sticky="ew")
        ...
        self.value.grid(row=1, column=0, padx=16, sticky="ew")
        ...
        self.detail.grid(row=2, column=0, padx=16, pady=(3, 14), sticky="ew")
...
class _ActivityRow(ctk.CTkFrame):
    def __init__(self, master, title: str, detail: str, when: datetime | None, tone: str):
        ...
        ctk.CTkLabel(
            ...
            text_color=color,
            font=(theme.FONT, 9),
        ).grid(row=0, column=0, rowspan=2, padx=(0, 8), sticky="n")
        ctk.CTkLabel(
            self,
            text=title,
            text_color=theme.TEXT,
        ...
        ctk.CTkLabel(
            ...
            font=(theme.FONT, 9),
            anchor="w",
        ).grid(row=1, column=1, pady=(2, 0), sticky="ew")
        ctk.CTkLabel(
            self,
            text=_format_when(when),
            text_color=theme.TEXT_DIM,
            font=(theme.FONT, 9),
            anchor="e",
        ).grid(row=0, column=2, rowspan=2, padx=(16, 0), sticky="e")
...
class DashboardPage(BasePage):
    title = "Home"
    subtitle = "A simple view of your protection"
    ...
    def __init__(self, master, controller, **kwargs):
        ...
        self.status_panel.grid(
            row=0, column=0, columnspan=3, padx=8, pady=(4, 12), sticky="ew"
        )
        ...
        self.status_message = ctk.CTkLabel(
            self.status_panel,
            text="AutoGuard is checking protection services.",
            text_color=theme.TEXT_MUTED,
            font=(theme.FONT, 11),
            anchor="w",
        )
        ...
        self.quick_scan = ctk.CTkButton(
            ...
            text_color=theme.BG,
            font=(theme.FONT, 10, "bold"),
            command=self.controller.start_quick_scan,
        )
        ...
        for col, key in enumerate(("Last Scan", "Threats", "Quarantine")):
            block = _SummaryBlock(self.body, key)
            block.grid(row=1, column=col, padx=8, pady=(0, 14), sticky="nsew")
            self.summaries[key] = block

        protection_header = ctk.CTkLabel(
            self.body,
            text="Protection",
            text_color=theme.TEXT,
            font=(theme.FONT, 15, "bold"),
        ...
        protection_box.grid(row=3, column=0, columnspan=3, padx=8, sticky="ew")
        protection_box.grid_columnconfigure(0, weight=1)
        self.protection_rows = {
            "real_time": _ProtectionRow(
                protection_box,
                "Real-time protection",
                "Watches supported folders for new and changed files.",
            ),
            "usb": _ProtectionRow(
                protection_box,
                "USB protection",
                "Checks removable drives when they are connected.",
            ),
            "scheduled": _ProtectionRow(
                protection_box,
                "Scheduled scanning",
                "Runs automatic Quick and Full scans on schedule.",
            ),
        }
        for row, widget in enumerate(self.protection_rows.values()):
            widget.grid(row=row, column=0, padx=16, pady=(13 if row == 0 else 9, 13), sticky="ew")

        activity_header = ctk.CTkFrame(self.body, fg_color="transparent")
        activity_header.grid(row=4, column=0, columnspan=3, padx=8, pady=(24, 7), sticky="ew")
        ...
        ctk.CTkButton(
            ...
            font=(theme.FONT, 9, "bold"),
            command=lambda: self.controller.navigate_to("activity"),
        ).grid(row=0, column=1, sticky="e")

        self.recent = ctk.CTkFrame(
            self.body,
            fg_color=theme.SURFACE,
            border_width=1,
            border_color=theme.BORDER,
        ...
        self.recent.grid(row=5, column=0, columnspan=3, padx=8, pady=(0, 14), sticky="ew")
    ...
    def handle_message(self, message) -> None:
        if message.kind == "dashboard_data":
            self._render(message.payload["result"])
        elif message.kind == "scan_started":
            self._set_status(
                "scanning",
                "Scanning",
                "AutoGuard is currently checking your files.",
            )
    ...
    def _set_status(self, key: str, title: str, message: str) -> None:
        color, background, icon = _STATE_STYLE.get(
            key, (theme.TEXT_MUTED, theme.SURFACE_ALT, "…")
        )
    ...
    def _render(self, data) -> None:
        ...
        self.summaries["Threats"].set(
            "No threats need review" if review_count == 0 else f"{review_count} need review",
            "Nothing needs your attention" if review_count == 0 else "Open Threats to review",
        )

        quarantine_count = data["quarantined_count"]
        self.summaries["Quarantine"].set(
            f"{quarantine_count} isolated file{'s' if quarantine_count != 1 else ''}",
            "Stored safely away from their original locations" if quarantine_count else "No files currently isolated",
        )
