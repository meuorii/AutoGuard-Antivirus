...
class ScanPage(BasePage):
    ...
    def __init__(self, master, controller, **kwargs):
        ...
        actions = ctk.CTkFrame(self.body, fg_color=theme.SURFACE, border_width=1, border_color=theme.BORDER, corner_radius=theme.RADIUS); actions.grid(row=0, column=0, padx=8, pady=6, sticky="ew")
    ...
    def handle_message(self, message):
        if message.kind in ("scan_started", "scan_counting"):
            if message.kind == "scan_started": self.counts = {k: 0 for k in self.counts}; self.log.delete("1.0", "end")
            self.progress.start(message.payload.get("path", "Scan"))
        elif message.kind == "scan_total":
            self.progress.set_total(message.payload.get("total", 0), message.payload.get("path", ""))
        elif message.kind == "scan_result":
            status, detection, path = message.payload["status"], message.payload.get("detection"), message.payload.get("path", "")
            ...
            self.progress.update_progress(message.payload.get("processed", sum(self.counts.values())), message.payload.get("total", 0), self.counts, path)
