...
class ScanProgressPanel(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.grid_columnconfigure(0, weight=1); self._discovered = self._processed = 0; self._mode = "manual"
        ...
        self.mode_badge = ctk.CTkLabel(h, text="IDLE", font=(theme.FONT, 9, "bold"), text_color=theme.ACCENT, fg_color=theme.SURFACE_ALT, corner_radius=6, padx=9, pady=3); self.mode_badge.grid(row=0, column=1, padx=(8, 10), sticky="e")
        self.percent = ctk.CTkLabel(h, text="0%", font=(theme.FONT, 13, "bold"), text_color=theme.ACCENT, anchor="e"); self.percent.grid(row=0, column=2, sticky="e")
        ...
        for i, key in enumerate(("scanned", "skipped", "suspicious", "dangerous", "errors")):
            f.grid_columnconfigure(i, weight=1); lbl = ctk.CTkLabel(f, text=f"0\n{key.title()}", font=(theme.FONT, 10), text_color=theme.TEXT_MUTED); lbl.grid(row=0, column=i, sticky="ew"); self.labels[key] = lbl

    def start(self, label: str, scan_type: str = "manual") -> None:
        self._discovered = self._processed = 0; self._mode = scan_type or "manual"
        names = {"manual": "Custom Scan", "quick": "Quick Scan", "full": "Full Scan", "startup": "Startup Scan", "scheduled": "Scheduled Scan", "usb": "USB Scan", "real_time": "Real-Time Scan"}
        self.title.configure(text=names.get(self._mode, self._mode.replace("_", " ").title())); self.mode_badge.configure(text=self._mode.replace("_", " ").upper()); self.detail.configure(text=f"Discovering & scanning • {label}"); self.percent.configure(text="0%"); self.work.configure(text="0 processed • 0 discovered"); self.progress.configure(mode="determinate"); self.progress.set(0)
    ...
    def update_discovery(self, discovered: int, processed: int = 0, current_path: str = "") -> None: self._discovered = max(self._discovered, int(discovered), int(processed)); self._processed = max(self._processed, int(processed)); self._paint(current_path)
    ...
    def finish(self, message: str) -> None: self.progress.set(1); self.percent.configure(text="100%"); self._discovered = max(self._discovered, self._processed); self.work.configure(text=f"{self._processed:,} processed • {self._discovered:,} discovered"); self.title.configure(text=f"{self._mode.replace('_', ' ').title()} complete"); self.mode_badge.configure(text="COMPLETED"); self.detail.configure(text=message)