...
class ConfirmDialog(ctk.CTkToplevel):
    def __init__(self, master, title: str, message: str, confirm_text: str = "Confirm", danger: bool = False, on_confirm=None):
        ...
        self.title(title); self.geometry("430x220"); self.resizable(False, False)
        ...
        ctk.CTkButton(row,text=confirm_text,fg_color=theme.DANGER if danger else theme.ACCENT,
                      hover_color=theme.DANGER_DARK if danger else theme.ACCENT_HOVER,
                      text_color="#07110F" if not danger else theme.TEXT,command=go).pack(side="right",padx=(0,8))
...
class RestoreDialog(ctk.CTkToplevel):
    def __init__(self, master, original_path: str, on_restore):
        ...
        ctk.CTkLabel(self,text="AutoGuard will verify the isolated file and re-evaluate it with current signatures before restoration.",wraplength=505,justify="left",text_color=theme.TEXT_MUTED,font=(theme.FONT,11)).pack(anchor="w",padx=24)
