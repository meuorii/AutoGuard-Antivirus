import argparse, json, logging; from pathlib import Path
from app.config import AppConfig; from app.database import Database; from app.detector import Detector; from app.incidents import IncidentService; from app.matching_cleanup import MatchingCopyCleanup; from app.quarantine import QuarantineService; from app.scanner import Scanner; from app.signatures import load_signatures; from app.threat_trail import ThreatTrail

def main() -> None:
    config = AppConfig()
    parser = argparse.ArgumentParser(description="AutoGuard Phase 7 scanner with exact Matching-Copy Cleanup")
    ...
    logging.getLogger(__name__).info("Phase 7 initialized with %d signatures", len(signatures))
    print("AutoGuard Phase 7 initialized."); print(f"Database: {config.database_path}"); print(f"Quarantine directory: {config.quarantine_dir}"); print(f"Logs: {config.logs_dir}"); print(f"Signatures loaded: {len(signatures)}")
    if args.path is not None:
        ...
        scanner = Scanner(Detector(signatures), trail, max_file_size_bytes=args.max_bytes, incidents=incidents, quarantine=quarantine, matching_cleanup=matching_cleanup)
        ...
        for cleanup in scanner.last_matching_cleanup_results:
            print(f"Matching cleanup: searched={len(cleanup.locations_searched)}, examined={cleanup.candidates_examined}, exact={cleanup.exact_matches_found}, quarantined={cleanup.matches_quarantined}, already_contained={cleanup.already_contained_matches}, failures={len(cleanup.failures)}, inaccessible={len(cleanup.inaccessible_files)}")
