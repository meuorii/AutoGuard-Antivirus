...
def main() -> None:
    config = AppConfig()
    parser = argparse.ArgumentParser(description="AutoGuard Phase 4 scanner with history")
    ...
    logging.getLogger(__name__).info("Phase 4 initialized with %d signatures", len(signatures))
    print("AutoGuard Phase 4 initialized."); print(f"Database: {config.database_path}"); print(f"Quarantine directory: {config.quarantine_dir}"); print(f"Logs: {config.logs_dir}"); print(f"Signatures loaded: {len(signatures)}")
