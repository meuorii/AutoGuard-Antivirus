...
def main() -> None:
    config = AppConfig()
    parser = argparse.ArgumentParser(description="AutoGuard Phase 5 scanner with threat incidents")
    ...
    logging.getLogger(__name__).info("Phase 5 initialized with %d signatures", len(signatures))
    print("AutoGuard Phase 5 initialized."); print(f"Database: {config.database_path}"); print(f"Quarantine directory: {config.quarantine_dir}"); print(f"Logs: {config.logs_dir}"); print(f"Signatures loaded: {len(signatures)}")
